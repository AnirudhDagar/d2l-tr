...
burada $\odot$ vektörlerin Hadamard ürününü gösterir. $\mathbf{P} \in \mathbb{R}^{m \times k}$ ve $\mathbf{Q} \in \mathbb{R}^{n \times k}$ sırasıyla kullanıcı ve öğeye gizli matrise çekirdek gölet. $\mathbf{p}_u \in \mathbb{R}^{ k}$ $P$ ve $\mathbf{q}_i \in \mathbb{R}^{ k}$ $\mathbf{q}_i \in \mathbb{R}^{ k}$, $P$ ve $\mathbf{q}_i \in \mathbb{R}^{ k}$, $Q$ satırının $i^\mathrm{th}$ sırasıdır. $\alpha$ ve 7322936363616 satırıdır. 14, çıkış katmanının aktivasyon işlevini ve ağırlığını gösterir . $\hat{y}_{ui}$ kullanıcısının $u$ öğesine verebileceği tahmin puanı $i$. 
...
## Değerlendirici Bu bölümde, eğitim ve test setlerini oluşturmak için zaman stratejisine göre bölünmeyi benimsedik. Verilen kesme $\ell$ ($\ text {Hit} @\ ell$) and area under the ROC curve (AUC) are used to assess the model effectiveness.  Hit rate at given position $\ ell$ for each user indicates that whether the recommended item is included in the top $\ ell$ sıralamasında verilen isabet oranı dahil olmak üzere iki değerlendirme önlemi. Resmi tanım şöyledir: 

$$
\text{Hit}@\ell = \frac{1}{m} \sum_{u \in \mathcal{U}} \textbf{1}(rank_{u, g_u} <= \ell),
$$

burada $\textbf{1}$, zemin gerçeği öğesi üst $\ell$ listesinde yer alıyorsa, aksi takdirde sıfıra eşittir. $rank_{u, g_u}$, $u$ kullanıcısının $g_u$ öğesinin tavsiye listesinde yer alan hakikat öğesinin $g_u$ sıralamasını gösterir (İdeal sıralama 1'dir). $m$ kullanıcı sayısı. $\mathcal{U}$ kullanıcı ayarıdır. 
