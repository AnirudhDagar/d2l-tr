...
Aşağıdaki örnekte gösterildiği gibi, daha yüksek boyutlardaki eyer noktaları daha da sinsidir. $f(x, y) = x^2 - y^2$ işlevini düşünün. Bu onun eyer noktası vardır $(0, 0)$. Bu, $y$ ile ilgili olarak maksimum ve $x$ ile ilgili minimum. Dahası, bu matematiksel mülkün adını aldığı bir eyer gibi görünüyor*.
...
1. Biz simetrik rastgele matris olduğunu varsayalım $\mathbf{M}$ girdileri $M_{ij} = M_{ji}$ her bazı olasılık dağılımından çizilir $p_{ij}$. Ayrıca, $p_{ij}(x) = p_{ij}(-x)$, yani dağılımın simetrik olduğunu varsayalım (ayrıntılar için bkz. :cite:`Wigner.1958`).
    1. Özdeğerler üzerindeki dağılımın da simetrik olduğunu kanıtlayın. Yani, herhangi bir özvektör için $\mathbf{v}$, ilişkili özdeğer $\lambda$'ün $P(\lambda > 0) = P(\lambda < 0)$'yı karşılama olasılığı.
