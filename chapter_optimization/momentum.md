...
Daha önce olduğu gibi $f$ en az $(0, 0)$. Bu fonksiyon $x_1$ yönünde *çok* düzdür. Bu yeni işlevde daha önce olduğu gibi degrade iniş gerçekleştirdiğimizde ne olacağını görelim. $0.4$ öğrenme oranı seçiyoruz.
...
İnşaat gereği, $x_2$ yönündeki degrade* çok daha yüksektir ve yatay $x_1$ yönünden çok daha hızlı değişir. Böylece iki istenmeyen seçenek arasında sıkıştık: küçük bir öğrenme hızı seçersek, çözümün $x_2$ yönünde sapmamasını sağlarız, ancak $x_1$ yönünde yavaş yakınsama ile eyerleniriz. Tersine, büyük bir öğrenme hızı ile $x_1$ yönünde hızla ilerliyoruz ancak $x_2$'te farklılaşıyoruz. Aşağıdaki örnek, $0.4$'den $0.6$'e kadar öğrenme hızında hafif bir artıştan sonra bile neler olduğunu göstermektedir. $x_1$ yönündeki yakınsama iyileşir ancak genel çözüm kalitesi çok daha kötüdür.
...
Hatırlayın o $\mathbf{v}_t = \sum_{\tau = 0}^{t-1} \beta^{\tau} \mathbf{g}_{t-\tau, t-\tau-1}$. Limitte terimler $\sum_{\tau=0}^\infty \beta^\tau = \frac{1}{1-\beta}$'ya kadar ekler. Başka bir deyişle, $\eta$ büyüklüğünde bir adım atmak yerine degrade iniş veya stokastik degrade iniş boyutu $\frac{\eta}{1-\beta}$ aynı zamanda bir adım atarken, potansiyel olarak çok daha iyi davranmış bir iniş yönü ile uğraşırken. Bunlar bir arada iki fayda. $\beta$'ün farklı seçenekleri için ağırlıklandırmanın nasıl davrandığını göstermek için aşağıdaki şemayı göz önünde bulundurun.
...
Bu genel bir kuadratik fonksiyondur. Pozitif kesin matrisler için $\mathbf{Q} \succ 0$, yani, pozitif özdeğerlere sahip matrisler için bu minimum değer $b - \frac{1}{2} \mathbf{c}^\top \mathbf{Q}^{-1} \mathbf{c}$ ile $\mathbf{x}^* = -\mathbf{Q}^{-1} \mathbf{c}$ değerinde bir minimize sahiptir. Bu nedenle $h$'i yeniden yazabiliriz 
...
$\mathbf{Q}$ pozitif kesin olduğundan $\mathbf{Q} = \mathbf{O}^\top \boldsymbol{\Lambda} \mathbf{O}$ ile $\mathbf{Q} = \mathbf{O}^\top \boldsymbol{\Lambda} \mathbf{O}$ üzerinden bir ortogonal (rotasyon) matrisi $\mathbf{O}$ ve pozitif özdeğerlerin diyagonal matrisi $\boldsymbol{\Lambda}$ diyagonal matrisi için ayrıştırılabilir. Bu, çok basitleştirilmiş bir ifade elde etmek için $\mathbf{x}$'dan $\mathbf{z} := \mathbf{O} (\mathbf{x} - \mathbf{Q}^{-1} \mathbf{c})$'ye değişken değişikliği yapmamıza olanak tanır: 

$$h(\mathbf{z}) = \frac{1}{2} \mathbf{z}^\top \boldsymbol{\Lambda} \mathbf{z} + b'.$$

İşte $b' = b - \frac{1}{2} \mathbf{c}^\top \mathbf{Q}^{-1} \mathbf{c}$. $\mathbf{O}$ yalnızca bir ortogonal matris olduğundan bu durum degradeleri anlamlı bir şekilde bozmaz. $\mathbf{z}$ degrade iniş açısından ifade olur 
...
$|1 - \eta \lambda| < 1$ $|1 - \eta \lambda| < 1$ bu optimizasyon üstel bir hızda birleştiğinde $t$ adımlardan sonra $x_t = (1 - \eta \lambda)^t x_0$ sahibiz. Bu, $\eta \lambda = 1$'e kadar $\eta$ öğrenme oranını artırdıkça yakınsama oranının başlangıçta nasıl arttığını gösterir. Bunun ötesinde şeyler ayrılıyor ve $\eta \lambda > 2$ için optimizasyon sorunu ayrılıyor.
...
$\mathbf{R}$ yöneten yakınsama davranışını göstermek için $\mathbf{R}$ kullandık. $t$ adımlarından sonra başlangıç tercihi $[v_0, x_0]$ $\mathbf{R}(\beta, \eta, \lambda)^t [v_0, x_0]$ olur. Bu nedenle, yakınsama hızını belirlemek $\mathbf{R}$'un özdeğerlerine kalmıştır. Harika bir animasyon için [Distill post](https://distill.pub/2017/momentum/) of :cite:`Goh.2017` ve ayrıntılı analiz için :cite:`Flammarion.Bach.2015` bölümüne bakın. Bir $0 < \eta \lambda < 2 + 2 \beta$ momentum yakınsama olduğunu gösterebilir. Bu, degrade iniş için $0 < \eta \lambda < 2$ ile karşılaştırıldığında daha geniş bir uygulanabilir parametre aralığıdır. Ayrıca, genel olarak $\beta$'nin büyük değerlerinin arzu edildiğini de göstermektedir. Daha fazla ayrıntı, adil miktarda teknik detay gerektirir ve ilgilenen okuyucunun orijinal yayınlara danışmasını öneririz. 
