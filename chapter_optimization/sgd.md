...
Derin öğrenmede, objektif işlev genellikle eğitim veri kümelerindeki her örnek için kayıp fonksiyonlarının ortalamasıdır. $n$ örneklerinden oluşan bir eğitim veri kümesi göz önüne alındığında, $f_i(\mathbf{x})$'in $\mathbf{x}$'nın parametre vektörü olduğu endeks $i$'ün eğitim örneğine göre kayıp fonksiyonu olduğunu varsayıyoruz. Sonra objektif fonksiyona varıyoruz 
...
Degrade iniş kullanılıyorsa, her bağımsız değişken yineleme için hesaplama maliyeti $\mathcal{O}(n)$'tür ve $n$ ile doğrusal olarak büyür. Bu nedenle, eğitim veri kümesi daha büyük olduğunda, her yineleme için degrade iniş maliyeti daha yüksek olacaktır. 

Stokastik degrade iniş (SGD), her yinelemede hesaplama maliyetini düşürür. Stokastik degrade iniş her yinelemede, rastgele veri örnekleri için $i\in\{1,\ldots, n\}$ dizini eşit olarak örnekleriz ve $\mathbf{x}$'i güncelleştirmek için $\nabla f_i(\mathbf{x})$ degradeyi hesaplarız: 

$$\mathbf{x} \leftarrow \mathbf{x} - \eta \nabla f_i(\mathbf{x}),$$

burada $\eta$ öğrenme oranıdır. Her yineleme için hesaplama maliyetinin $\mathcal{O}(n)$'ten degrade inişinin $\mathcal{O}(n)$'ten $\mathcal{O}(1)$ sabitine düştüğünü görebiliriz. Dahası, stokastik degrade $\nabla f_i(\mathbf{x})$'ün $\nabla f(\mathbf{x})$'in tam degradenin tarafsız bir tahmini olduğunu vurgulamak istiyoruz, çünkü 
...
$f(\boldsymbol{\xi}, \mathbf{x})$ nesnel fonksiyonunun $\boldsymbol{\xi}$'ün tümü için $\mathbf{x}$'te dışbükey olduğunu varsayalım. Daha somut olarak, stokastik degrade iniş güncellemesini göz önünde bulunduruyoruz: 
...
beklenen risk ve $R^*$ ile ilgili olarak minimum $\mathbf{x}$. Son izin $\mathbf{x}^*$ minimizer (biz $\mathbf{x}$ tanımlandığı etki alanı içinde var olduğunu varsayalım). Bu durumda $t$ zamanında $t$ ve risk minimizer $\mathbf{x}^*$ arasındaki mesafeyi izleyebilir ve zamanla iyileşip iyileşmediğini görebiliriz: 
...
Çoğunlukla $\mathbf{x}_t$ ve $\mathbf{x}^*$ arasındaki mesafenin beklentide* ile nasıl değiştiğiyle ilgileniyoruz. Aslında, herhangi bir belirli adım dizisi için, karşılaştığımız $\boldsymbol{\xi}_t$'ün hangisine bağlı olarak mesafe artabilir. Bu yüzden nokta ürününü bağlamamız gerekiyor. $f$ herhangi bir dışbükey fonksiyon için $f(\mathbf{y}) \geq f(\mathbf{x}) + \langle f'(\mathbf{x}), \mathbf{y} - \mathbf{x} \rangle$ tüm $\mathbf{x}$ ve $\mathbf{y}$ için, dışbükeylik ile 

$$f(\boldsymbol{\xi}_t, \mathbf{x}^*) \geq f(\boldsymbol{\xi}_t, \mathbf{x}_t) + \left\langle \mathbf{x}^* - \mathbf{x}_t, \partial_{\mathbf{x}} f(\boldsymbol{\xi}_t, \mathbf{x}_t) \right\rangle.$$
:eqlabel:`eq_sgd-f-xi-xstar`

:eqref:`eq_sgd-L` ve :eqref:`eq_sgd-f-xi-xstar`'yı :eqref:`eq_sgd-xt+1-xstar`'e hem eşitsizlikleri :eqref:`eq_sgd-xt+1-xstar`'e takarak $t+1$'deki parametreler arasındaki mesafeye aşağıdaki gibi ulaşırız: 

$$\|\mathbf{x}_{t} - \mathbf{x}^*\|^2 - \|\mathbf{x}_{t+1} - \mathbf{x}^*\|^2 \geq 2 \eta_t (f(\boldsymbol{\xi}_t, \mathbf{x}_t) - f(\boldsymbol{\xi}_t, \mathbf{x}^*)) - \eta_t^2 L^2.$$
:eqlabel:`eqref_sgd-xt-diff`

Bu, mevcut kayıp ve optimal kayıp arasındaki fark $\eta_t L^2/2$'ten ağır bastığı sürece ilerleme kaydetmemiz anlamına gelir. Bu farkın sıfıra yakınsama bağlı olduğu için $\eta_t$ öğrenme oranının da *kaybolması gerektiği izlenir. 
...
Jensen'in eşitsizliği ile ($i=t$, :eqref:`eq_jensens-inequality` içinde :eqref:`eq_jensens-inequality` içinde $\alpha_i = \eta_t/\sum_{t=1}^T \eta_t$ ayarı) ve $R$ dışbükeyliği ile bu $E[R(\mathbf{x}_t)] \geq E[R(\bar{\mathbf{x}})]$, 
