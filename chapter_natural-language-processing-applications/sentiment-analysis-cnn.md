...
:numref:`fig_conv1d`'te gösterildiği gibi, tek boyutlu durumda, konvolüsyon penceresi giriş tensör boyunca soldan sağa doğru kayar. Kayma sırasında, belirli bir konumdaki evrişim penceresinde bulunan giriş subtensör (örn. :numref:`fig_conv1d` yılında $0$ ve $1$) ve çekirdek tensör (örneğin, :numref:`fig_conv1d`'te $1$ ve $2$) elementer olarak çarpılır. Bu çarpımların toplamı, çıkış tensörünün karşılık gelen pozisyonunda tek skaler değeri (örneğin, :numref:`fig_conv1d`'te $0\times1+1\times2=2$) verir. 

Aşağıdaki `corr1d` fonksiyonunda tek boyutlu çapraz korelasyon uyguluyoruz. Bir giriş tensör `X` ve bir çekirdek tensör `K` göz önüne alındığında, çıkış tensörünü `Y` döndürür.
...
Yukarıdaki tek boyutlu çapraz korelasyon uygulamasının çıkışını doğrulamak için `X` giriş tensörünü ve :numref:`fig_conv1d`'ten `K`'dan `K`'yı inşa edebiliriz.
...
:numref:`fig_conv1d` ve :numref:`fig_conv1d_channel`'daki her iki çıkışta da yalnızca bir kanal vardır. :numref:`subsec_multi-output-channels`'te açıklanan çoklu çıkış kanallarına sahip iki boyutlu kıvrımlarla aynı şekilde, tek boyutlu kıvrımlar için birden fazla çıkış kanalı da belirtebiliriz. 
...
$d$ boyutlu vektörler tarafından temsil edilen $n$ belirteçleri ile tek bir metin dizisi için, giriş tensörünün kanal genişliği, yüksekliği ve sayısı sırasıyla $n$, $1$ ve $d$'tür. textCNN modeli, girdiyi çıktıya aşağıdaki gibi dönüştürür: 
