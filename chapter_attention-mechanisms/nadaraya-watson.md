...
İşleri basit tutmak için, aşağıdaki regresyon sorununu ele alalım: $\{(x_1, y_1), \ldots, (x_n, y_n)\}$ giriş-çıkış çiftlerinden oluşan bir veri kümesi verilen $\{(x_1, y_1), \ldots, (x_n, y_n)\}$, herhangi bir yeni giriş $x$ için $\hat{y} = f(x)$ çıktısını tahmin etmek için $f$ nasıl öğrenilir? 
...
burada $K$ bir *çekirdek*. :eqref:`eq_nadaraya-watson`'teki tahmin ediciye *Nadaraya-Watson çekirdek regresyonu* denir. Burada çekirdeklerin ayrıntılarına dalmayacağız. :numref:`fig_qkv`'teki dikkat mekanizmalarının çerçevesini hatırlayın. Dikkat perspektifinden, :eqref:`eq_nadaraya-watson`'ü daha genelleştirilmiş bir formda*dikkat havuzu* ile yeniden yazabiliriz: 

$$f(x) = \sum_{i=1}^n \alpha(x, x_i) y_i,$$
:eqlabel:`eq_attn-pooling`

burada $x$ sorgu ve $(x_i, y_i)$ anahtar-değer çiftidir. :eqref:`eq_attn-pooling` ve :eqref:`eq_avg-pooling`'ü karşılaştırarak, buradaki dikkat biriktirme $y_i$ değerlerinin ağırlıklı bir ortalamasıdır. *dikkat ağırlığı* :eqref:`eq_attn-pooling`'teki $\alpha(x, x_i)$, $x$ sorgu ve $\alpha$ tarafından modellenen anahtar $x_i$ arasındaki etkileşime dayanarak karşılık gelen $y_i$ değerine atanır. Herhangi bir sorgu için, tüm anahtar-değer çiftleri üzerindeki dikkat ağırlıkları geçerli bir olasılık dağılımıdır: negatif değildir ve bir adede kadar toplamlar. 
...
:eqref:`eq_nadaraya-watson-gaussian`'te, $x$ numaralı sorgunun $x$'ya daha yakın olduğu bir anahtar $x_i$
...
Örnek olarak, :eqref:`eq_nadaraya-watson-gaussian`'ten biraz farklı, aşağıdaki sorgu $x$ ve $x_i$ tuşu arasındaki mesafe, öğrenilebilir bir parametre $w$ ile çarpılır: 
...
İlk mini batch $n$ matrisleri $a\times b$ şekil $a\times b$ ve ikinci mini toplu işlemin $n$ matrisleri $\mathbf{Y}_1, \ldots, \mathbf{Y}_n$ şekil $b\times c$ şeklinin 73229363619 içerdiğini varsayalım. Onların toplu matris çarpımı $n$ şekil $a\times c$ matrisleri $\mathbf{X}_1\mathbf{Y}_1, \ldots, \mathbf{X}_n\mathbf{Y}_n$ ile sonuçlanır. Bu nedenle, [**iki şekil tensör ($n$, $a$, $b$) ve ($n$, $b$, $c$) verildiğinde, toplu matris çarpma çıktılarının şekli ($n$, $a$, $c$) .**]
