...
:numref:`sec_rnn_scratch`'te kelimeleri temsil etmek için tek sıcak vektörler kullandık (karakterler kelimelerdir). Sözlükteki farklı kelimelerin sayısının (sözlük boyutu) $N$ olduğunu ve her kelimenin $0$ ila $N−1$ arasında farklı bir tamsayıya (dizin) karşılık geldiğini varsayalım. İndeksi $i$ olan herhangi bir sözcük için tek sıcak vektör temsilini elde etmek için, tüm 0s ile bir uzunluk-$N$ vektörü oluşturuyoruz ve öğeyi $i$ için 1 konumuna ayarlıyoruz. Bu şekilde, her kelime $N$ uzunluğunda bir vektör olarak temsil edilir ve doğrudan sinir ağları tarafından kullanılabilir. 
...
Skip-gram modelinde, her kelimenin koşullu olasılıkları hesaplamak için iki adet $d$ boyut-vektör gösterimi vardır. Daha somut olarak, sözlükte indeks $i$ olan herhangi bir kelime için, sırasıyla $\mathbf{v}_i\in\mathbb{R}^d$ ve $\mathbf{u}_i\in\mathbb{R}^d$ ile iki vektörünü, *center* kelimesi ve bir *context* kelimesi olarak kullanıldığında belirtin. $c$ (sözlükte indeks $c$ ile) orta kelime verilen $w_o$ (sözlükte indeks $c$ ile) herhangi bir bağlam sözcük oluşturma koşullu olasılık (dizin $c$) vektör nokta ürünlerinde bir softmax işlemi ile modellenebilir: 
...
Kelimelerin sürekli çanta modelinde birden fazla bağlam kelimeleri bulunduğundan, bu bağlam kelime vektörleri koşullu olasılığın hesaplanmasında ortalamalanır. Özellikle, sözlükte indeks $i$ olan herhangi bir kelime için, sırasıyla $\mathbf{v}_i\in\mathbb{R}^d$ ve $\mathbf{u}_i\in\mathbb{R}^d$, bir *context* kelimesi ve bir *center* sözcük olarak kullanıldığında iki vektörünü (anlamlar atlama gram modelinde değiştirilir). Çevredeki bağlam sözcükleri $w_{o_1}, \ldots, w_{o_{2m}}$ (sözlükte indeks $o_1, \ldots, o_{2m}$ ile) $w_{o_1}, \ldots, w_{o_{2m}}$ (dizin $o_1, \ldots, o_{2m}$ sözlükte) verilen herhangi bir merkez sözcük üretme koşullu olasılık (dizin $o_1, \ldots, o_{2m}$ sözlükte) tarafından modellenebilir 
...
$T$ uzunluğunda bir metin dizisi göz önüne alındığında, burada adım $t$ kelimesi $w^{(t)}$ olarak gösterilir. Bağlam penceresi boyutu $m$ için, sürekli kelime çantası modelinin olasılık fonksiyonu, bağlam kelimeleri verilen tüm merkez sözcükleri oluşturma olasılığıdır: 
...
Farklılaşma sayesinde, biz herhangi bir bağlam kelime vektör göre onun gradyanı elde edebilirsiniz $\mathbf{v}_{o_i}$ ($i = 1, \ldots, 2m$) olarak 
