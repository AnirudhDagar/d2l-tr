...
Orijinal BERT farklı model boyutları :cite:`Devlin.Chang.Lee.ea.2018` iki versiyonu vardır. Temel model ($\text{BERT}_{\text{BASE}}$) 768 gizli ünite (gizli boyut) ve 12 kendine dikkat kafalı 12 katman (transformatör kodlayıcı blokları) kullanır. Büyük model ($\text{BERT}_{\text{LARGE}}$), 1024 gizli ünite ve 16 kendine dikkat kafalı 24 katman kullanır. Özellikle, birincisinin 110 milyon parametresi varken, ikincisi 340 milyon parametreye sahiptir. Kolayca gösteri için, 2 katman, 128 gizli ünite ve 2 özdikkat kafası kullanarak küçük bir BERT tanımlıyoruz.
...
BERT ön eğitiminden sonra, tek metin, metin çiftleri veya bunlardaki herhangi bir belirteci temsil etmek için kullanabiliriz. Aşağıdaki işlev, `tokens_a` ve `tokens_b` içindeki tüm belirteçler için BERT (`net`) temsillerini döndürür.
