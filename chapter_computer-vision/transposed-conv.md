...
çekirdek aracılığıyla, böylece girişten daha büyük bir çıktı üretir. `X`'den `X` giriş tensörünü ve `K`'dan `K`'yı temel iki boyutlu transpoze edilmiş evrişim işleminin [**yukarıdaki uygulamanın çıkışını doğrulayın**] için inşa edebiliriz.
...
Birden fazla giriş ve çıkış kanalı için, dönüştürülmüş evrişim normal konvolüsyon ile aynı şekilde çalışır. Girişte $c_i$ kanallara sahip olduğunu ve dönüştürülmüş evrimin her giriş kanalına bir $k_h\times k_w$ çekirdek tensör atadığını varsayalım. Birden fazla çıkış kanalı belirtildiğinde, her çıkış kanalı için bir $c_i\times k_h\times k_w$ çekirdeğine sahip olacağız. 

Tüm olarak, $\mathsf{X}$'yı $\mathsf{X}$'yı $\mathsf{Y}=f(\mathsf{X})$ çıktısını vermek için $f$'ü evrimsel bir tabaka halinde beslersek ve $f$ ile aynı hiperparametrelere sahip bir transpoze edilmiş evrimsel katman oluşturursak, $\mathsf{X}$'daki kanal sayısı hariç, $\mathsf{X}$'daki kanal sayısı hariç, $g(Y)$ aynı şekle sahip olacaktır $\mathsf{X}$. Bu, aşağıdaki örnekte gösterilebilir.
...
Daha sonra, evrişim çekirdeği `K`'ü çok sayıda sıfır içeren seyrek ağırlık matrisi `W` olarak yeniden yazıyoruz. Ağırlık matrisinin şekli ($4$, $9$) olup, sıfır olmayan elemanların evrişim çekirdeği `K`'den geldiği yerdir.
...
9 uzunluğunda bir vektör elde etmek için giriş `X` satırını satır birleştirin. Daha sonra `W`'nın matris çarpımı ve vektörleştirilen `X`'ün uzunluğu 4 vektörü verir. Yeniden şekillendirdikten sonra, yukarıdaki orijinal evrişim işleminden `Y` ile aynı sonucu elde edebiliriz: matris çarpımlarını kullanarak kıvrımlar uyguladık.
...
Matrisleri çarparak evrimi uygulamayı düşünün. Bir giriş vektörü $\mathbf{x}$ ve bir ağırlık matrisi $\mathbf{W}$ göz önüne alındığında, evrimin ileri yayılma fonksiyonu, giriş ağırlık matrisi ile çarpılarak ve bir vektör $\mathbf{y}=\mathbf{W}\mathbf{x}$ çıktısıyla uygulanabilir. Geri yayılma zincir kuralını ve $\nabla_{\mathbf{x}}\mathbf{y}=\mathbf{W}^\top$'i izlediğinden, konvolüsiyonun geri yayılma fonksiyonu, girişi transpoze ağırlık matrisi $\mathbf{W}^\top$ ile çarpılarak uygulanabilir. Bu nedenle, transpoze edilmiş konvolüsyonel tabaka sadece ileriye yayılma fonksiyonunu ve dönme tabakasının geri yayılma fonksiyonunu değiştirebilir: ileri yayılma ve geri yayılma fonksiyonları sırasıyla $\mathbf{W}^\top$ ve $\mathbf{W}$ ile giriş vektörünü çarpar. 
...
* $\mathsf{X}$'yı $\mathsf{X}$ çıkış yapmak için $f$'ü evrimsel bir katmana beslersek ve $\mathsf{X}$'daki kanal sayısı olan çıkış kanallarının sayısı hariç $f$ ile aynı hiperparametrelere sahip bir transpoze evrimsel katman $g$ oluşturursak, $\mathsf{X}$ ile aynı şekle sahip olacaktır.
