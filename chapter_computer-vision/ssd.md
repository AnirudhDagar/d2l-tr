...
:numref:`fig_ssd`, tek çekimli çoklu kutu algılama tasarımına genel bir bakış sağlar. Bu model esas olarak bir baz ağdan ve ardından birkaç çok ölçekli özellik harita bloğundan oluşur. Temel ağ, giriş görüntüsünden özellikleri ayıklamak içindir, bu nedenle derin bir CNN kullanabilir. Örneğin, orijinal tek çekimli çok kutulu algılama kağıdı sınıflandırma katmanı :cite:`Liu.Anguelov.Erhan.ea.2016`'ten önce kesilmiş bir VGG ağını benimser, ResNet de yaygın olarak kullanılmıştır. Tasarımımız sayesinde, daha küçük nesneleri algılamak için daha fazla bağlantı kutusu üretmek için temel ağ çıkışını daha büyük özellik haritaları yapabiliriz. Daha sonra, her çok ölçekli özellik eşleme bloğu önceki bloktan özellik eşlemelerinin yüksekliğini ve genişliğini azaltır (örn. yarı yarıya) ve özellik eşlemelerinin her biriminin giriş görüntüsündeki alıcı alanını artırmasını sağlar. 
...
Nesne sınıflarının sayısı $q$ olsun. Daha sonra çapa kutularının $q+1$ sınıfları vardır, burada sınıf 0 arka plandır. Bazı ölçeklerde, özellik haritalarının yüksekliği ve genişliğinin sırasıyla $h$ ve $w$ olduğunu varsayalım. $a$ çapa kutuları, bu özellik haritalarının her mekansal konumu ile merkezi olarak oluşturulduğunda, toplam $hwa$ çapa kutularının sınıflandırılması gerekir. Bu durum genellikle büyük olasılıkla ağır parametreleme maliyetleri nedeniyle tamamen bağlı katmanlarla sınıflandırmayı mümkün kılar. :numref:`sec_nin`'te sınıfları tahmin etmek için evrimsel katman kanallarını nasıl kullandığımızı hatırlayın. Tek çekim çoklu kutu algılama, model karmaşıklığını azaltmak için aynı tekniği kullanır. 

Özellikle, sınıf tahmini katmanı, özellik eşlemelerinin genişliğini veya yüksekliğini değiştirmeden bir kıvrımsal katman kullanır. Bu şekilde, özellik haritalarının aynı uzamsal boyutlarında (genişlik ve yükseklik) çıkışlar ve girişler arasında bire bir yazışma olabilir. Daha somut olarak, herhangi bir uzamsal konumdaki çıkış özelliği eşlemelerinin kanalları ($x$, $y$) giriş özelliği eşlemelerinin merkezli tüm bağlantı kutuları için sınıf tahminlerini temsil eder ($x$, $y$). Geçerli tahminler üretmek için $a(q+1)$ çıkış kanalı olmalıdır, burada aynı uzamsal konum için $i(q+1) + j$ dizine sahip çıkış kanalı $i(q+1) + j$ ($0 \leq i < a$) bağlantı kutusu $i$ ($0 \leq i < a$) sınıfı tahminini temsil eder. 
...
Aşağıdaki örnekte, `Y2` yüksekliğinin ve genişliğinin `Y1`'in yarısı olduğu aynı mini batch için `Y1` ve `Y2` olmak üzere iki farklı ölçekte özellik eşlemeleri oluşturuyoruz. Örnek olarak sınıf tahminini ele alalım. Her birim için sırasıyla `Y1` ve `Y2`'teki 5 ve 3 çapa kutularının oluşturulduğunu varsayalım. Nesne sınıflarının sayısının 10 olduğunu varsayalım. `Y1` ve `Y2` özellik haritaları için sınıf tahmini çıkışlarındaki kanal sayısı sırasıyla $5\times(10+1)=55$ ve $3\times(10+1)=33$'dır. Burada her iki çıkış şekli de (toplu iş boyutu, kanal sayısı, yükseklik, genişlik).
...
Bu şekilde, `Y1` ve `Y2`'ün kanal, yükseklik ve genişliklerde farklı boyutlara sahip olmasına rağmen, bu iki tahmin çıktısını aynı minibatch için iki farklı ölçekte birleştirebiliriz.
...
Temel ağ bloğu, giriş görüntülerinden özellikleri ayıklamak için kullanılır. Basitlik açısından, her bloktaki kanal sayısını iki katına çıkaran üç altörnekleme bloğundan oluşan küçük bir temel ağ oluşturuyoruz. $256\times256$ giriş görüntüsü göz önüne alındığında, bu temel ağ bloğu $32 \times 32$ özellik eşlemelerini ($256/2^3=32$) çıkışlar.
...
1. Kayıp işlevini geliştirerek tek çekim çoklu kutu algılamasını geliştirebilir misiniz? Örneğin, $L_1$ norm kaybını öngörülen ofsetler için düzgün $L_1$ norm kaybı ile değiştirin. Bu kayıp fonksiyonu, hiperparametre $\sigma$ tarafından kontrol edilen pürüzsüzlük için sıfır etrafında bir kare işlevi kullanır:
...
Ayrıca, deneyde sınıf tahmini için çapraz entropi kaybı kullandık: $p_j$'ya göre $p_j$'ya göre zemin doğruluk sınıfı $j$ için tahmin edilen olasılık, çapraz entropi kaybı $-\log p_j$'dur. Ayrıca fokal kaybı :cite:`Lin.Goyal.Girshick.ea.2017` kullanabilirsiniz: hiperparametreler $\gamma > 0$ ve $\alpha > 0$, bu kayıp olarak tanımlanır: 
