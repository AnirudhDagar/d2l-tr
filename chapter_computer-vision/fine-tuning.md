...
:begin_tab:`mxnet`
Önceden eğitilmiş kaynak model örneği iki üye değişken içerir: `features` ve `output`. Birincisi, çıktı katmanı dışındaki modelin tüm katmanlarını içerir ve ikincisi modelin çıkış katmanıdır. Bu bölümün temel amacı, tüm katmanların ancak çıktı tabakasının model parametrelerinin ince ayarlanmasını kolaylaştırmaktır. Kaynak modelin üye değişkeni `output` aşağıda gösterilmiştir.
:end_tab:
